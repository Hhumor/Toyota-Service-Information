(function() {
    ...
    function loadContentHtml(contentId) {
        const contentUrl = CONTENTS_BASE_URL + contentId + '.html';
        const contentFrame = document.getElementById('content-frame');
        const contentBody = document.getElementById('contentsBody');
        
        if (!contentFrame || !contentBody) {
        ...
        fetch(contentUrl)
            ...
            .then(html => {
                ...
                const bodyContent = doc.querySelector('#contentsBody');
                
                if (bodyContent) {
                    contentBody.innerHTML = bodyContent.innerHTML;
                    hideLoadingPlaceholder();
                    initContentScripts();
                } else {
                    contentBody.textContent = '内容加载失败';
                    hideLoadingPlaceholder();
                }
            })
            .catch(error => {
                console.error('加载内容失败:', error);
                contentBody.textContent = '内容加载失败: ' + error.message;
                hideLoadingPlaceholder();
            });
    }
    ...
    function showLoadingPlaceholder() {
        ...
        if (contentFrame) contentFrame.classList.remove('hidden');
    }
